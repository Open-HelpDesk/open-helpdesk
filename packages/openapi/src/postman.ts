...
function toPostmanUrl(serverUrl: string, path: string) {
  ...
  return {
    ...
    host: ["{{baseUrl}}"],
    path: path.replace(/^\//, "").replace(/\{(\w+)\}/g, ":$1").split("/").filter(Boolean),
...
export function toPostmanCollection(doc: Json): Json {
  ...
  for (const [path, itemRaw] of Object.entries(paths)) {
    ...
    for (const method of METHODS) {
      ...
      const schema = (body?.["application/json"] as Json | undefined)?.["schema"] as Json | undefined;
  ...
  return {
    info: {
      ...
      description:
        ...
        "Set `token` to an API key from Settings → API & webhooks, and `baseUrl` to your workspace.",
